...
from src.fetcher import get_videos, get_video_metadata_by_id
from src.processor import sort_videos
from src.logging_util import load_downloaded_ids, append_line
from src.downloader import download_video
from src.transcoder import TranscodeWorker, TranscodeJob
...
def backfill_existing_media(output_dir: str, queue_ref, counters: Dict[str, int], crf_quality: int) -> None:
	# Remove partials
...
def main() -> None:
	...
	parser.add_argument("--channel-url", help="Channel or playlist URL (required unless --interactive)")
	parser.add_argument("--downloader", default=DOWNLOADER_DEFAULT, choices=["yt-dlp", "youtube-dl"], help="Downloader to use (youtube-dl is deprecated)") 
	parser.add_argument("--after-date", default=None, help="Only include videos after YYYYMMDD")
	parser.add_argument("--keyword-filter", default=None, help="Only include videos with title containing this keyword")
	parser.add_argument("--crf-quality", type=int, default=23, help="Video quality CRF value (18-30, lower=higher quality)")
	parser.add_argument("--output-path", default=OUTPUT_DIR_DEFAULT, help="Output directory base")
	parser.add_argument("--max-retries", type=int, default=MAX_RETRIES_DEFAULT, help="Maximum download retries")
	...
	print(f"[0/7] Transcoding: CRF {args.crf_quality} (quality), Medium preset (balanced speed)")

	# Backfill cleanup for existing files
	counters = {"queued": 0}
	backfill_existing_media(args.output_path, transcode_queue, counters, args.crf_quality)
	if counters["queued"]:
	...
	filter_info = []
	if is_single_video:
		print(f"[3/7] Single video detected - skipping filters")
		...
		videos = sort_videos(videos, after_date=args.after_date, keyword_filter=args.keyword_filter)
	...
	for index, v in enumerate(videos, start=1):
		...
		if video_id in downloaded_ids:
			print(f"({index}/{total}) Skipping already downloaded: {title} [{video_id}]")
			continue

		channel_dir = args.output_path
		ensure_dir(channel_dir)
		print(f"({index}/{total}) Downloading: {title} [{video_id}] via {args.downloader} -> {channel_dir}")
		path = download_video(v, args.downloader, channel_dir, retries=args.max_retries)
		if path:
			print(f"    ✓ Downloaded: {os.path.basename(path)}")
			if path.lower().endswith(".mp4"):
				transcode_queue.put(TranscodeJob(source_path=path, output_dir=channel_dir, mode="remux", title=title, uploader=uploader, upload_date=upload_date, webpage_url=webpage_url, crf_quality=args.crf_quality))
				queued_transcodes += 1
				print(f"    ↳ queued for remux metadata: {os.path.basename(path)}")
			else:
				transcode_queue.put(TranscodeJob(source_path=path, output_dir=channel_dir, mode="transcode", title=title, uploader=uploader, upload_date=upload_date, webpage_url=webpage_url, crf_quality=args.crf_quality))
				queued_transcodes += 1
	...
	print("[6/7] Waiting for background post-processing to finish (see transcode.log for details)...")
	transcode_queue.join()
	worker.stop()
	print("[6/7] Post-processing completed.")

	print(f"[7/7] Summary: Successes={successes}, Failures={failures}, PostProcessed={queued_transcodes}")
	try:
		import subprocess  # local import to avoid global dependency
		subprocess.run(["notify-send", "YouTube Downloader", f"Done. Success: {successes}, Failures: {failures}"], check=False)
	except Exception:
		pass
