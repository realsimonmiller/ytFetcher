...
def _build_ffmpeg_transcode(
    ...
    if has_thumb:
        ...
        cmd += [
            ...
            "libx264",
            "-crf",
            str(crf_quality),  # Use the CRF quality parameter
            "-preset",
            "medium",  # Changed from "slow" to "medium" for faster processing
            "-pix_fmt",
            "yuv420p",
            "-c:a:0",
            "aac",
        ...
        cmd += [
            ...
            str(crf_quality),  # Use the CRF quality parameter
            "-preset",
            "medium",  # Changed from "slow" to "medium"
            "-pix_fmt",
            "yuv420p",
            "-c:a:0",
            "aac",
...
class ProgressBar:
    ...
    def render(self) -> str:
        ...
        progress = (
            min(1.0, self.current_frame / self.total_frames)
            if self.total_frames > 0
            else 0
        )
        bar_width = 30
    ...
    def run(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                print(
                    f"[transcode] Starting: {os.path.basename(job.source_path)} -> {os.path.basename(dst)} ({job.mode})"
                )
                ...
                with subprocess.Popen(
                    ...
                    progress_bar = ProgressBar(
                        f"Transcoding {os.path.basename(job.source_path)}", total_frames
                    )
                    ...
                    print(
                        f"[transcode] Completed ({status}): {os.path.basename(job.source_path)} -> {os.path.basename(dst)}"
                    )
                    if success:
                        # Replace source if remux; cleanup intermediates
                        try:
                            if job.mode == "remux" and job.source_path.lower().endswith(
                                ".mp4"
                            ):
                        ...
                        if os.path.exists(thumb_jpg):
                            try:
                                ...
                                pass
                    self._append_log(
                        f"{status}: {os.path.basename(job.source_path)} -> {os.path.basename(dst)}"
                    )
                    if job.on_complete:
