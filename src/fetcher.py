...
def get_videos(url: str) -> List[Dict[str, Any]]:
	...
	except Exception as e:  # fallback to youtube_dl if available
		try:
			import youtube_dl as ytdlp  # type: ignore
			print("⚠️  yt-dlp failed, falling back to youtube-dl (may be outdated)")
			...
			if "yt initial data" in str(exc):
				error_msg += "\n\nThis usually means youtube-dl is outdated and can't handle modern YouTube."
