...
def _build_command(video: Dict[str, Any], downloader: str, output_path: str) -> list[str]:
	...
	if downloader == "yt-dlp":
		...
		return [
			...
			output_template,
			"-f",
			"(bestvideo[height>=720]/bestvideo)+(bestaudio)/best[height>=720]/best",
			"--write-thumbnail",
			url,
		]
	else:
...
def download_video(video: Dict[str, Any], downloader: str, output_path: str, retries: int = 3) -> Optional[str]:
